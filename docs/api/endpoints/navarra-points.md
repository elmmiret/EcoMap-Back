# GET /api/navarra/points - Puntos de Reciclaje (Navarra)

Devuelve el listado completo de puntos de reciclaje publicados por el Gobierno de Navarra (fuente: CKAN Open Data). La respuesta es un array de objetos tal cual es devuelto por el dataset oficial.
...
[
  {
    "id": "67890",
    "name": "Punto Limpio",
    "address": "Av. Navarra 45, Pamplona",
    "lat": 42.818,
    "lon": -1.644,
    "materials": ["pilas", "aceite", "RAEE"],
...
```bash
curl -X GET http://localhost:3001/api/navarra/points
```

### JavaScript/TypeScript (fetch)

```javascript
async function getNavarraPoints() {
  const res = await fetch('http://localhost:3001/api/navarra/points');
...
Future<List<Map<String, dynamic>>> getNavarraPoints() async {
  final uri = Uri.parse('http://localhost:3001/api/navarra/points');
